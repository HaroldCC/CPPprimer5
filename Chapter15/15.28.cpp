...
> Created Time    : 2020年06月30日  19时43分49秒
************************************************************************/
#include <string>
using std::string;
#include <iostream>
...
public:
    ...
    double net_price(size_t buyBooksCount) const override { std::cout << "bulk_quote.net_prince()" << std::endl; }
...
int main(int argc, const char *argv[])
{
    ...
    for (unsigned i = 1; i != 10; ++i)
        qv.push_back(Bulk_quote("sss", i * 10.1, 10, 0.3));
    ...
    /**
      ...
      * @retval 
      */
    vector<std::shared_ptr<Quote>> shv;
    for (unsigned i = 1; i != 10; ++i)
        shv.push_back(std::make_shared<Bulk_quote>(Bulk_quote("sss", i * 10.1, 10, 0.3)));

    double totalP = 0;
    for (auto i : shv)
        totalP += i->net_price(20);
