...
{
	if (str.size() <= 0 || n_count <= 0 || i_start >= str.size())
		return "";
...
{
	if (str.size() <= 0 || str_left.size() <= 0 || str_right.size() <= 0 || str.size() < str_left.size() || str.size() < str_right.size())
		return "";
...
{
	if (str.size() <= 0 || str_begin.size() <= 0)
		return false;
...
{
	if (str.size() <= 0 || str_sub.size() <= 0 || str_sub.size() > str.size())
		return false;
